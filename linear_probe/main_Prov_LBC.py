...
# Dataset
argparser.add_argument('--dataset_csv',         type=str, default='/home/aindra/prov-gigapath/merged_embeddings.csv', help='The csv file indicating input samples and labels')
...
# Output
argparser.add_argument('--output_dir',          type=str, default='outputs/LBC', help='Output directory')
...
def train(model,
    ...
    for i, (embed, target) in enumerate(infinite_train_loader):
        # print('target',target)
        # if target == -1:  # Skip samples with invalid targets
        #     continue
...
class EmbeddingDataset(Dataset):
    def __init__(self, dataset_csv: str, zip_path: str, split: str = 'train', z_score=False, processor=None):
        ...
        self.samples = split_df['path'].tolist()  # Paths to embedding files
    ...
    def __getitem__(self, index):
        ...
        # Extract the last 4 words from the path excluding the extension
        # parts = list(Path(img_path ).parts[-5:])  # Get the last 4 parts
        # parts[-1] = Path(parts[-1]).stem  # Remove the .webp extension from the last part
        # last_4 = "_".join(parts)  # Join parts with underscores
         # Construct the embedding path
        # embed_path = os.path.join(self.zip_path, str(target), img_path)
        embed_path = os.path.join(img_path)
        # print (embed_path)
        # Load the embedding tensor from the file
        embed = torch.load(embed_path)  # Ensure the embeddings are saved in PyTorch format (.pt)
        # try:
        #     # Try loading the embedding tensor from the file
        #     embed = torch.load(embed_path)  # Ensure the embeddings are saved in PyTorch format (.pt)
        # except FileNotFoundError:
        #     # Handle missing files gracefully
        #     print(f"Warning: Embedding file not found: {embed_path}. Skipping...")
        #     # Return a placeholder tensor (e.g., zeros) and ignore this sample
        #     embed = torch.zeros(1, 512)  # Adjust the dimensions to match your embeddings
        #     target = -1  # Use -1 for invalid targets
